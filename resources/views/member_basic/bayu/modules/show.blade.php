...
<div class="max-w-6xl mx-auto pb-20">
    ...
    {{-- Tombol Kembali --}}
    <a href="{{ route('member.modules.index') }}" class="inline-flex items-center gap-2 text-sm font-bold text-slate-500 dark:text-slate-400 mb-8 hover:text-lime-600 transition group">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-8 lg:p-12">
            <h1 class="text-3xl font-bold text-slate-900 dark:text-white mb-6">{{ $module->title }}</h1>

            <div class="prose max-w-none text-slate-600 dark:text-slate-400 mb-10">
                {!! nl2br(e($module->description)) !!}
            </div>

            @if($module->attachment_link)
            <div class="mb-10 p-6 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-2xl flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-white dark:bg-slate-800 rounded-xl shadow-sm flex items-center justify-center text-rose-500">
                    ...
                    <div>
                        <h4 class="font-bold text-slate-900 dark:text-white">Buku Panduan Ekstra</h4>
                        <p class="text-xs text-slate-500 dark:text-slate-400">PDF / Dokumen Referensi</p>
                    </div>
                </div>
            ...
            <div class="p-8 bg-gradient-to-br from-amber-50 to-lime-50 border-2 border-lime-400 rounded-2xl text-center">
                ...
                <h3 class="text-2xl font-black text-slate-900 dark:text-white mb-2">Selamat! Piagammu Sudah Siap!</h3>
                <p class="text-slate-600 dark:text-slate-400 mb-6">Nilai Akhir: <strong class="text-lime-600 text-xl">{{ $pivot->pivot->quiz_score }} / 100</strong></p>
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    <a href="{{ route('member.certificate.download', $certCode) }}" target="_blank"
                       class="inline-flex items-center gap-3 px-8 py-4 bg-slate-900 text-white font-black uppercase tracking-widest rounded-2xl hover:bg-lime-500 hover:text-slate-900 dark:text-white transition-all shadow-lg hover:shadow-xl hover:-translate-y-1">
                        📥 Unduh Piagam PDF
                    </a>
                    <a href="{{ route('certificate.verify', $certCode) }}" target="_blank"
                       class="inline-flex items-center gap-3 px-8 py-4 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 text-slate-700 font-bold uppercase tracking-widest rounded-2xl hover:border-lime-400 transition-all">
                        🔍 Verifikasi Piagam
                    </a>
                </div>
            </div>
            ...
            <div class="text-center">
                ...
                @else
                    <div class="p-6 bg-slate-50 dark:bg-slate-900 rounded-2xl text-left">
                        <h3 class="font-black text-slate-900 dark:text-white text-lg mb-1">📝 Kumpulkan Tugas Kamu</h3>
                        <p class="text-sm text-slate-500 dark:text-slate-400 mb-5">Selesaikan materi di atas, lalu submit link hasil kerjamu (Google Drive, Notion, GitHub, dll).</p>
                        <form action="{{ route('member.modules.submit', $module->id) }}" method="POST" class="space-y-4">
                            @csrf
                            <div>
                                <label class="block text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Link Tugas / Hasil Kerja *</label>
                                <input type="url" name="submission_link" required
                                       class="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-lime-400"
                                       placeholder="https://drive.google.com/..."
                                       value="{{ old('submission_link') }}">
                            </div>
                            <div>
                                <label class="block text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Catatan / Keterangan (Opsional)</label>
                                <textarea name="submission_note" rows="3"
                                          class="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-lime-400"
                                          placeholder="Ceritakan singkat apa yang kamu kerjakan...">{{ old('submission_note') }}</textarea>
                            </div>
                            <button type="submit"
                                    class="w-full py-4 bg-slate-900 text-white font-black uppercase tracking-widest rounded-2xl hover:bg-lime-500 hover:text-slate-900 dark:text-white transition-all shadow-lg text-sm">
                                📤 Kumpulkan Tugas
                            </button>
                        </form>
                    </div>
