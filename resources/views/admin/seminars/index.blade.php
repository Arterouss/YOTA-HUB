...
@section('title', 'Kelola Seminar & Webinar')
    ...
    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
        ...
        <tbody class="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-900">
            ...
            <tr>
                ...
                <td class="px-6 py-4 text-center">
                    <a href="{{ route('admin.seminars.participants', $seminar->id) }}" class="inline-flex items-center px-3 py-1 bg-slate-100 dark:bg-slate-800 hover:bg-lime-100 dark:hover:bg-lime-900/30 text-slate-700 dark:text-lime-400 rounded-lg text-xs font-bold transition-colors">
                        👥 Lihat Pendaftar
                    </a>
                </td>
                <td class="px-6 py-4">
                    <span class="px-2 inline-flex text-xs leading-5 font-bold rounded-full {{ $seminar->status == 'Open' ? 'bg-lime-100 text-lime-800' : ($seminar->status == 'Full' ? 'bg-orange-100 text-orange-800' : 'bg-red-100 text-red-800') }}">
                        {{ strtoupper($seminar->status) }}
                    </span>
                </td>
                <td class="px-6 py-4 text-right text-sm font-medium space-x-2">
                    <a href="{{ route('admin.seminars.edit', $seminar->id) }}" class="text-blue-600 hover:text-blue-900 dark:hover:text-blue-400">Edit</a>
                    <form action="{{ route('admin.seminars.destroy', $seminar->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin hapus acara ini? Semua data pendaftar juga akan hilang!');">
                        @csrf
                        @method('DELETE')
