...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
        <form action="{{ route('admin.knowledge.update', $article->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ $article->title }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="reading_time" value="{{ $article->reading_time }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3" min="1">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-xs font-black text-slate-500 uppercase tracking-wider mb-2">Link Cover Image (Opsional)</label>
                    <input type="url" name="featured_image" value="{{ $article->featured_image }}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-xs font-black text-slate-500 uppercase tracking-wider mb-2">Ringkasan (Summary Singkat)</label>
                    <textarea name="summary" rows="3" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">{{ $article->summary }}</textarea>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-xs font-black text-slate-500 uppercase tracking-wider mb-2">Isi Konten Artikel</label>
                    <textarea name="content" rows="15" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-mono text-sm leading-relaxed">{{ $article->content }}</textarea>
                </div>
