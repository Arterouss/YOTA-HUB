<div class="flex flex-col h-full bg-[#0F172A] transition-all duration-300">
    ...
    <div class="h-20 flex items-center px-6 border-b border-white/10">
        <div class="flex-shrink-0 w-10 h-10 bg-[#BEF264] rounded-xl flex items-center justify-center shadow-lg shadow-lime-500/20">
            <span class="text-slate-900 dark:text-white font-bold text-xl uppercase">Y</span>
        </div>
    ...
    <nav class="flex-1 px-4 py-8 space-y-1 overflow-y-auto">
        ...
        <p x-show="sidebarOpen" class="px-3 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] mb-3">Core Panel</p>

        <a href="#"
           class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all group
                  {{ request()->is('admin/dashboard*') ? 'bg-[#BEF264] text-slate-900 dark:text-white shadow-md' : 'text-slate-400 hover:bg-white dark:bg-slate-800/5 hover:text-white' }}">
        ...
        <div class="pt-6">
            ...
            <a href="{{ route('admin.users.index') }}" class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all group {{ request()->is('admin/master/users*') ? 'bg-[#BEF264] text-slate-900 dark:text-white shadow-md' : 'text-slate-400 hover:bg-white dark:bg-slate-800/5 hover:text-white' }}">
                <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="pt-6">
            <p x-show="sidebarOpen" class="px-3 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] mb-3">Security</p>

            <a href="#" class="flex items-center gap-4 px-4 py-3.5 rounded-xl text-slate-400 hover:bg-white dark:bg-slate-800/5 hover:text-white transition-all group">
                <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="p-4 bg-black/20">
        <button @click="sidebarOpen = !sidebarOpen"
                class="w-full flex items-center justify-center p-3 rounded-xl bg-white dark:bg-slate-800/5 text-slate-400 hover:text-[#BEF264] hover:bg-white dark:bg-slate-800/10 transition-all active:scale-95">
