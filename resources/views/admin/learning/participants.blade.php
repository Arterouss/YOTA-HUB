...
@section('title', 'Daftar Peserta: ' . $course->title)

@section('content')
<div class="sm:flex sm:items-center sm:justify-between mb-8">
    <div>
        <a href="{{ route('admin.learning.index') }}" class="text-xs font-bold text-slate-500 hover:text-lime-600 mb-2 inline-block">← KEMBALI KE DAFTAR KURSUS</a>
        <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Peserta: {{ $course->title }}</h2>
        <p class="mt-1 text-sm text-slate-500">
            Tipe Kursus: <span class="font-bold text-slate-700">{{ strtoupper($course->course_type) }}</span> 
            | Harga: <span class="font-bold text-slate-700">Rp{{ number_format($course->price, 0, ',', '.') }}</span>
        </p>
    </div>
</div>
    ...
    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
        <thead class="bg-slate-50 dark:bg-slate-800">
            <tr>
                ...
                <th class="px-6 py-4 text-left text-xs font-black text-slate-500 uppercase tracking-wider">Progress Belajar</th>
        ...
        <tbody class="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-900">
            ...
            <tr>
                <td class="px-6 py-4">
                    <div class="text-sm font-bold text-slate-900 dark:text-white">{{ $enrollment->user->name }}</div>
                </td>
                ...
                <td class="px-6 py-4 text-right text-sm font-medium">
                    @if($enrollment->payment_status == 'pending')
                        <form action="{{ route('admin.learning.verify', [$course->id, $enrollment->user->id]) }}" method="POST">
                            @csrf
            ...
            <tr>
                <td colspan="5" class="px-6 py-12 text-center text-slate-500 border-2 border-dashed border-slate-200">
                    Belum ada pendaftar untuk kursus ini.
                </td>
            </tr>
            @endforelse
        </tbody>
