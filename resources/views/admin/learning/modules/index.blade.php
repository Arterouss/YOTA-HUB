...
<div class="sm:flex sm:items-center sm:justify-between mb-8">
    <div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.learning.index') }}" class="p-2 bg-slate-100 dark:bg-slate-800 rounded-lg hover:bg-slate-200 text-slate-600 dark:text-slate-400">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            </a>
            <h2 class="text-2xl font-bold text-slate-800">Manajemen Silabus & Materi Video</h2>
        </div>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400 ml-11">Kursus: <strong>{{ $course->title }}</strong></p>
    </div>
...
<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
    <table class="min-w-full divide-y divide-slate-200">
        <thead class="bg-slate-50 dark:bg-slate-900">
            <tr>
                <th class="px-6 py-4 text-left text-xs font-black text-slate-500 dark:text-slate-400 uppercase">Urutan</th>
                <th class="px-6 py-4 text-left text-xs font-black text-slate-500 dark:text-slate-400 uppercase">Judul Bab</th>
                <th class="px-6 py-4 text-left text-xs font-black text-slate-500 dark:text-slate-400 uppercase">Tipe Akses</th>
                <th class="px-6 py-4 text-left text-xs font-black text-slate-500 dark:text-slate-400 uppercase">Video URL</th>
                <th class="px-6 py-4 text-right text-xs font-black text-slate-500 dark:text-slate-400 uppercase">Aksi</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-slate-200">
            ...
            <tr>
                ...
                <td class="px-6 py-4">
                    <div class="text-sm font-bold text-slate-900 dark:text-white">{{ $module->module_title }}</div>
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400 truncate max-w-[200px]">
                    @if($module->video_url)
